    ...
    public class RecordPartialInvalidTest : GeneratorCodeFixVerifier
    {
        ...
        private class TestCases : TheoryDataProvider
        {
            public override IEnumerable<ITheoryDatum> GetDataSets()
            {
                ...
                SourceTuple GetRecordAttributeSourceTuple() => ("Record.cs", GenerateRecordAttributeDeclarationCodeFixProvider.RecordAttributeDeclarationSource(@namespace));
                string CreateMessageDiff(string before, string after)
                {
                    return RecordPartialInvalidDiagnostic.CreateMessageDiff(SyntaxFactory.ParseCompilationUnit(before), SyntaxFactory.ParseCompilationUnit(after));
                }
    ...
    partial class Person
    {
        ...
        public Person WithLastName(string LastName)
        {
            return new Person(FirstName, LastName);
                        ...
                        .AndFixedSameAsOld()
                        .GetAdditionalSource(1, out var additional)
                        .GetChangedSource(out var changed),
                        ExpectedDiagnostics = new[]
                        {
                            new DiagnosticResult(RecordPartialInvalidDiagnostic.Descriptor, typeName, CreateMessageDiff(additional, changed))
                            {
                                Locations = new[] { new DiagnosticResultLocation(filename, 5, 19) }
                            }
                        }
                    };
                }
